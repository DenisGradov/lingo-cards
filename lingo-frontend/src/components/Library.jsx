...
const Library = () => {
  const { selectedLanguage, getLanguageInfo } = useUserInfo();
  ...
  useEffect(() => {
    if (playlists){
      const sorted = playlists.filter(pl => pl.last_open_time !== 0)
          ...
          <div className="flex flex-col gap-y-[25px]">
            ...
            <ScrollContainer ref={scrollRef} onWheel={handleWheel}>
              <div className="flex gap-x-[18px] cursor-pointer">
                {sortedLastOpened.map(function (playlist, index) {
                  return (
                      <div
