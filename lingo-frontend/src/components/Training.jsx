            ...
            <div className="relative flex flex-col items-center w-full">
              {showWordList && (
                  <div className="absolute bottom-[150px] max-w-[550px]  w-[97%] m-auto flex flex-col flex-grow  inset-x-0 bg-white dark:bg-[#333560] shadow-lg rounded-lg  h-[200px] overflow-y-auto p-4">
                    <h3 className="text-lg font-semibold dark:text-white text-[#282950] mb-2">Your Words</h3>
                    <ul>
                      {wordsActual.map((word, index) => (
                          <li key={index} className="flex justify-between items-center border-b border-gray-200 dark:border-gray-700 py-2">
                            <span className="text-sm dark:text-white">{word.word}</span>
                            <div className="flex">
                              <span className="text-sm text-gray-600 dark:text-gray-400">{word.translation}</span>
                              <RiBallPenFill onClick={() => handleEditWord(word)} className="text-[#775CFF] text-[20px] mr-[16px] hover:opacity-60 cursor-pointer duration-300" />
                            </div>
                          </li>
                      ))}
                    </ul>
