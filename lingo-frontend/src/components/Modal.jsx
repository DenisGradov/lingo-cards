...
const Modal = () => {
    ...
    const handlePlaylistSelect = (playlistId) => {
        setSelectedPlaylist((prevSelected) => (prevSelected === playlistId ? null : playlistId));
    };
    ...
    const handleSubmit = async (e) => {
        ...
        if (contentType === 'word' || contentType === 'editWord') {
            ...
            if (!wordInfo.translation.trim()) newErrors.translation = 'The translation cannot be empty';
        } else if (contentType === 'playlist') {
            if (!playlistInfo.name.trim()) newErrors.name = 'Playlist name cannot be empty';
            if (!playlistInfo.description.trim()) newErrors.description = 'Description cannot be empty';
        }
        ...
        try {
            ...
            if (contentType === 'playlist') {
                ...
                const response = await editWord(selectedWord.id, { ...wordInfo, playlistId: selectedPlaylist });
                if (response) {
                    updatedPlaylists = response.playlists;
                } else {
                        ...
                        <ScrollContainer className="flex gap-x-4 overflow-auto">
                            <div className="flex gap-x-[10px] cursor-pointer">
                                {playlists.map((playlist, index) => (
                                    <div
                                        key={`playlist-${index}`}
                                        style={{ backgroundImage: `url('${languages[playlist.language_code]?.flag || './flags/default.webp'}')` }}
                                    ...
                                    >
                                        ...
                                        <div className="relative flex flex-col justify-center items-center text-center w-full h-full">
                                            <p className="text-xs font-medium mb-1 text-[#F3F7FF]">{playlist.name}</p>
